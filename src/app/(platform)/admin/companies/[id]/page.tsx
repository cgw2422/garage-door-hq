...
export default async function PlatformCompanyPage({
  ...
  const { organization, paymentVolumeCents, paymentCount, lastActivityAt, auditLog } = overview
  const subscription = organization.subscription

  return (
    <div className="space-y-4">
      <div>
        <Link href="/admin" className="text-[0.8125rem] font-semibold text-brand-600">
          ← All companies
        </Link>
        <h1 className="mt-1 text-2xl font-bold text-ink">{organization.name}</h1>
        <p className="text-sm text-ink-muted">
          {organization.slug} · created {formatDate(organization.createdAt)} · last activity{' '}
      ...
      <Card>
        ...
        {subscription ? (
          <DataGrid>
            ...
            <DataPoint
              label="Price"
              value={formatCents(subscription.priceCents, { currency: subscription.currency })}
            />
            <DataPoint label="Trial ends" value={formatDate(subscription.trialEndsAt)} />
            <DataPoint label="Period ends" value={formatDate(subscription.currentPeriodEnd)} />
            <DataPoint
              label="Complimentary until"
              value={formatDate(subscription.complimentaryUntil)}
            />
            <DataPoint label="Cancelled" value={formatDate(subscription.cancelledAt)} />
          </DataGrid>
        ) : (
      ...
      <Card>
        ...
        <DataGrid>
          ...
          <DataPoint label="Company size" value={organization.companySize.replace('_', ' ')} />
          <DataPoint
            label="Referred by"
            value={
      ...
      <div>
        <SectionHeading>Users ({organization.memberships.length})</SectionHeading>
        <Card padded={false}>
            ...
            <div key={membership.id}>
              ...
              <div className="flex items-start justify-between gap-3 px-4 py-3">
                <div className="min-w-0">
                  ...
                  <p className="truncate text-sm text-ink-muted">{membership.user.email}</p>
                ...
                <div className="flex shrink-0 flex-col items-end gap-1.5">
                  <Chip tone={membership.role === 'OWNER' ? 'brand' : 'neutral'}>
                    {ROLE_LABELS[membership.role]}
                  </Chip>
                  {!membership.isActive ? <Chip tone="danger">inactive</Chip> : null}
                </div>
              </div>
            </div>
          ))}
