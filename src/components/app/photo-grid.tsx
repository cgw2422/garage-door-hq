...
export function PhotoGrid({
  ...
  return (
    <ul className={cn('grid grid-cols-3 gap-1.5 p-3', className)}>
      {photos.map((photo) => (
        <li key={photo.id} className="relative aspect-square overflow-hidden rounded-[--radius-control] bg-surface-sunken">
          <Image
          ...
          />
          <span className="absolute bottom-0 inset-x-0 bg-navy-950/60 px-1.5 py-0.5 text-[0.625rem] font-semibold uppercase tracking-wide text-white">
            {photo.kind.replace('_', ' ')}
          </span>
        </li>
      ))}
    </ul>
